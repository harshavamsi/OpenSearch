...
final class AggColumnarPlan {
     ...
     */
    /**
     * Build a plan directly from a shard-side columnar carrier ({@link ColumnarTermsShardResult}),
     * whose columns already declare their key kind and metric shapes — no bucket introspection.
     */
    static AggColumnarPlan fromCarrier(ColumnarTermsShardResult carrier) {
        TermKeyKind keyKind = carrier.isLongKey() ? TermKeyKind.LONG : TermKeyKind.STRING;
        List<MetricEntry> metrics = new ArrayList<>(carrier.metrics().size());
        for (ColumnarTermsShardResult.MetricColumn col : carrier.metrics()) {
            metrics.add(new MetricEntry(col.name, kindOf(col.kind)));
        }
        return new AggColumnarPlan(carrier.getName(), keyKind, metrics);
    }

    private static MetricKind kindOf(org.opensearch.search.aggregations.metrics.ColumnarMetricSink.Kind k) {
        switch (k) {
            case MAX:
                return MetricKind.MAX;
            case MIN:
                return MetricKind.MIN;
            case SUM:
                return MetricKind.SUM;
            case AVG:
                return MetricKind.AVG;
            case VALUE_COUNT:
                return MetricKind.VALUE_COUNT;
            case CARDINALITY:
                return MetricKind.CARDINALITY;
            default:
                throw new IllegalStateException("unknown sink kind " + k);
        }
    }
    ...
    static Optional<AggColumnarPlan> detect(TransportResponse response) {
        ...
        if (only instanceof ColumnarTermsShardResult carrier) {
            // Shard-side columnar emit: the carrier already declares its columns; no bucket walk.
            return Optional.of(fromCarrier(carrier));
        }
