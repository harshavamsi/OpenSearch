 ...
 * @opensearch.internal
 */
public class SumAggregator extends NumericMetricsAggregator.SingleValue implements StarTreePreComputeCollector, ColumnarMetricSink {
    ...
    public void writeColumnarValue(long bucketOrd, ValueSink sink) {
        // Emit the value only (matches InternalSum's wire form); the Kahan compensation term is
        // shard-local and the coordinator fold re-runs compensated summation on merge.
        sink.putDouble(valuesSource == null || bucketOrd >= sums.size() ? 0.0 : sums.get(bucketOrd));
    }
