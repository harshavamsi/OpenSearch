...
public final class ColumnarTermsCodec {
    ...
    /**
     * Writes the non-bucket portion from a pre-assembled {@link TermsHeader}, emitting exactly the
     * same bytes as {@link #writeTermsHeader(InternalMappedTerms, StreamOutput)}. Used by the
     * shard-side columnar emit path, whose carrier is not an {@link InternalMappedTerms}.
     */
    public static void writeTermsHeader(TermsHeader h, StreamOutput out) throws IOException {
        out.writeString(h.name);
        out.writeGenericValue(h.metadata);
        h.reduceOrder.writeTo(out);
        h.order.writeTo(out);
        writeSize(h.requiredSize, out);
        out.writeVLong(h.minDocCount);
        out.writeZLong(h.docCountError);
        out.writeNamedWriteable(h.format);
        writeSize(h.shardSize, out);
        out.writeBoolean(h.showTermDocCountError);
        out.writeVLong(h.otherDocCount);
    }
