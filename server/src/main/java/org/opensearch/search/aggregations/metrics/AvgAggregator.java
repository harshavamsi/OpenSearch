 ...
 * @opensearch.internal
 */
class AvgAggregator extends NumericMetricsAggregator.SingleValue implements StarTreePreComputeCollector, ColumnarMetricSink {
    ...
    @Override
    public void writeColumnarValue(long bucketOrd, ValueSink sink) {
        // Emit sum + count (matches InternalAvg's wire form); compensation is shard-local and the
        // coordinator fold re-runs compensated summation on merge.
        if (valuesSource == null || bucketOrd >= sums.size()) {
            sink.putSumCount(0.0, 0L);
        } else {
            sink.putSumCount(sums.get(bucketOrd), counts.get(bucketOrd));
        }
    }
