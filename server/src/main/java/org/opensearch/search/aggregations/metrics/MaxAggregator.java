 ...
 * @opensearch.internal
 */
class MaxAggregator extends NumericMetricsAggregator.SingleValue implements StarTreePreComputeCollector, ColumnarMetricSink {
    ...
    public void writeColumnarValue(long bucketOrd, ValueSink sink) {
        // Matches buildAggregation: an untouched ordinal reads NEGATIVE_INFINITY, exactly what
        // InternalMax would carry.
        sink.putDouble(valuesSource == null || bucketOrd >= maxes.size() ? Double.NEGATIVE_INFINITY : maxes.get(bucketOrd));
    }
