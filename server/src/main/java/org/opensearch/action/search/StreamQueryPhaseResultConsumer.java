...
public class StreamQueryPhaseResultConsumer extends QueryPhaseResultConsumer {
    ...
    /**
     * Coordinator {@code SearchTask} id. Keys the per-query columnar terms folder in
     * {@link ColumnarTermsFolderFactory}. The Arrow plugin folds received batches into that folder
     * (transport thread), then this consumer finalizes it once at {@link #reduce()} and injects the
     * result. {@code -1} when no task id was supplied (non-streaming-transport paths / tests), in
     * which case no folder lookup happens and the standard buffer path is used.
     */
    ...
    public void close() {
        try {
            ...
            // Release any columnar terms folder for this query. Idempotent: the folder is also
            // removed at reduce(); this covers the failure/short-circuit paths that skip reduce().
            if (taskId != -1L) {
                ColumnarTermsFolderFactory.Folder folder = ColumnarTermsFolderFactory.remove(taskId);
                if (folder != null) {
                    folder.release();
                }
            }
    ...
    /**
     * Final reduce. When the Arrow columnar folder folded this query's terms batches transport-side
     * (placeholder QSRs carried empty aggs), the merged terms live in the per-query folder, not in
     * the buffered aggregations. Finalize the folder once and inject its result into the reduced
     * phase. When no folder is active this is exactly {@code super.reduce()}.
     */
    @Override
    public SearchPhaseController.ReducedQueryPhase reduce() throws Exception {
        SearchPhaseController.ReducedQueryPhase reduced = super.reduce();
        if (taskId == -1L) {
            return reduced;
        }
        ColumnarTermsFolderFactory.Folder folder = ColumnarTermsFolderFactory.remove(taskId);
        ...
        try {
            InternalAggregation foldedTerms = folder.finalizeAggregation(aggReduceContextBuilder().forFinalReduction());
            if (foldedTerms == null) {
                return reduced;
            }
            List<InternalAggregation> merged = new ArrayList<>();
            merged.add(foldedTerms);
            // Preserve any other top-level aggs that flowed through the normal buffer path
            // (defensive: v1 eligibility restricts the folded shape to a single top-level terms).
            if (reduced.aggregations != null) {
                for (Aggregation a : reduced.aggregations.asList()) {
                    if (a.getName().equals(foldedTerms.getName()) == false) {
                        merged.add((InternalAggregation) a);
                    }
                }
            }
            return reduced.withAggregations(InternalAggregations.from(merged));
        } finally {
            folder.release();
        }
    }
