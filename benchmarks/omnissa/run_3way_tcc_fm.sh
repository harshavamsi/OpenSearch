#!/usr/bin/env bash
# 3-way sampled benchmark on just the 5 TCC/filtered_metric queries that exercise
# the new streaming + columnar code paths.
set -uo pipefail
...
TS=$(date +%s)
OUT=${OUT:-/local/home/hvamsi/code/OpenSearch/.claude/worktrees/omnissa-streaming/benchmarks/omnissa/results/tcc_fm_3way_$TS}
INCLUDE_TASKS="cbp_tcc_urls_over_5,cbp_fm_urls_over_5,om_nested_low_card_tcc,om_nested_high_card_tcc,om_nested_high_card_fm"
mkdir -p "$OUT"
echo "OUT=$OUT"
