#!/usr/bin/env bash
# 2-way A/B: baseline vs columnar on one multi_terms query, to see whether
# the Arrow-columnar path helps the multi_terms shape specifically.
set -uo pipefail
...
TS=$(date +%s)
OUT=${OUT:-/local/home/hvamsi/code/OpenSearch/.claude/worktrees/omnissa-streaming/benchmarks/omnissa/results/multi_terms_2way_$TS}
INCLUDE_TASKS=${INCLUDE_TASKS:-om_multi_term_low_card_max}
mkdir -p "$OUT"
echo "OUT=$OUT"
